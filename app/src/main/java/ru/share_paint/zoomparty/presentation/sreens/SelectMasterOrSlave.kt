...
import ru.share_paint.zoomparty.presentation.sreens.setting.BriefInstructionOneDevice
...
import ru.share_paint.zoomparty.presentation.ui.theme.DownFamily
...
fun SelectMasterOrSlave(serviceViewModel: ServiceViewModel, navController: NavHostController) {
    ...
    Column(
        ...
        Spacer(modifier = Modifier.size(largePadding.dp))
        BriefInstructionOneDevice()
        Spacer(modifier = Modifier.size(largePadding.dp))
        BriefInstructionTwoDevice(navController)
...
fun ShowRole(role: BluetoothWorkProfile) {
    Text(text = role.mName, fontFamily = DownFamily, fontSize = 24.sp, color = CianLight)
}
